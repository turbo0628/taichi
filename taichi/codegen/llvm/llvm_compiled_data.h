...
class OffloadedTask {
 public:
  ...
  explicit OffloadedTask(const std::string &name = "",
                         ...
                         int grid_dim = 0,
                         int dynamic_shared_array_size = 0)
      : name(name), block_dim(block_dim), grid_dim(grid_dim), dynamic_shared_array_size(dynamic_shared_array_size) {};
  TI_IO_DEF(name, block_dim, grid_dim, dynamic_shared_array_size);
};
